...
//    }
    ...
    void setPositionOrientation(Vector3 pos, Vector3 lookat) {
      ...
      // 計算相機的右向量
      Vector3 right = Vector3.cross(up, forward);  // 右向量是上向量與視線方向的叉積
      right.normalize();                           // 歸一化右向量
  
      // 計算最終的上向量
      Vector3 finalUp = Vector3.cross(forward, right);  // 最終的上向量是視線方向與右向量的叉積
      ...
      worldView.setZAxis(forward);  // 第三列: -Forward 向量（視線方向的反向）
